...
class MultiMainTimingParams:
    ...
    def GET_HASH_EXT(self, parser_state):
        ...
        s = top_level_str
        hash_ext = "_" + (
            (hashlib.md5(s.encode("utf-8")).hexdigest())[0:4]
        )  # 4 chars enough?
        return hash_ext
...
class TimingParams:
    def __init__(self, inst_name, logic):
        ...
        self._slices = (
            []
        )  # Unless raw vhdl (no submodules), these are only ~approximate slices
    ...
    for logic_inst_name in parser_state.LogicInstLookupTable:
        ...
        if logic_i.func_name in _GET_ZERO_CLK_HASH_EXT_LOOKUP_cache:
            ...
            _GET_ZERO_CLK_HASH_EXT_LOOKUP_cache[
                ...
                ZeroClockTimingParamsLookupTable, parser_state
            )
...
def GET_ZERO_CLK_PIPELINE_MAP(inst_name, Logic, parser_state, write_files=True):
    ...
    # Get pipeline map
    zero_clk_pipeline_map = GET_PIPELINE_MAP(
        inst_name, Logic, parser_state, ZeroClockLogicInst2TimingParams
    )
    ...
    def PIPELINE_DONE():
        ...
        for output in logic.outputs:
            if (output not in wires_driven_by_so_far) or (
                wires_driven_by_so_far[output] == None
            ):
                if print_debug:
        ...
        for feedback_var in logic.feedback_vars:
            if (feedback_var not in wires_driven_by_so_far) or (
                wires_driven_by_so_far[feedback_var] == None
            ):
                if print_debug:
        ...
        for state_reg in logic.state_regs:
            if logic.state_regs[state_reg].is_volatile == False:
                ...
                if (global_wire not in wires_driven_by_so_far) or (
                    wires_driven_by_so_far[global_wire] == None
                ):
                    if print_debug:
            ...
            if logic.state_regs[state_reg].is_volatile:
                ...
                if volatile_global_wire in logic.wire_driven_by:
                    ...
                    if (volatile_global_wire not in wires_driven_by_so_far) or (
                        wires_driven_by_so_far[volatile_global_wire] == None
                    ):
                        if print_debug:
                            print(
        ...
        for wire in logic.wires:
            if (
                wire not in wires_driven_by_so_far
                ...
                if not already_fully_driven and not incorrect_stage_for_submodule:
                    ...
                    if submodule_has_all_inputs_driven:
                        ...
                        if is_zero_clk_has_delay:
                            ...
                            rv.zero_clk_submodule_start_offset[
                                submodule_inst
                            ...
                            if submodule_latency > 0:
                                print(
                                    "What non zero latency for pipeline map? FIX MEhow? Probably dont fix now since fractional delay dont/cant make sense yet?"
                                )
                            ...
                            rv.zero_clk_submodule_end_offset[
                                submodule_inst
                            ] = abs_delay_end_offset
...
def SLICE_DOWN_HIERARCHY_WRITE_VHDL_PACKAGES(
  ...
  """
    ...
    if len(logic.submodule_instances) > 0:
        ...
        zero_clk_pipeline_map = GET_ZERO_CLK_PIPELINE_MAP(
        ...
        submodule_insts = zero_clk_pipeline_map.zero_clk_per_delay_submodules_map[
            delay_offset
        ]
        ...
        if delay_offset - 1 in zero_clk_pipeline_map.zero_clk_per_delay_submodules_map:
            prev_submodule_insts = (
                zero_clk_pipeline_map.zero_clk_per_delay_submodules_map[
                    delay_offset - 1
                ]
            )
        ...
        for submodule_inst in submodule_insts:
            ...
            if not submodule_timing_params.params_are_fixed:
                ...
                start_offset = zero_clk_pipeline_map.zero_clk_submodule_start_offset[
                    submodule_inst
                ]
    ...
    if coarse_only:
        ...
        if target_mhz is None:
            ...
            if starting_guess_latency is None:
                print(
                    "Starting a coarse sweep incrementally from zero latency comb. logic.",
                    flush=True,
                )
    ...
    while not sweep_state.met_timing:
        ...
        while keep_try_for_timing_params:
            ...
            sweep_state.multimain_timing_params.TimingParamsLookupTable = (
                GET_ZERO_CLK_TIMING_PARAMS_LOOKUP(parser_state)
            )
        ...
        for reported_clock_group in sweep_state.timing_report.path_reports:
            ...
            for main_inst in main_insts:
                ...
                if better_mhz or better_latency:
                    ...
                    sweep_state.inst_sweep_state[main_inst].latency_to_mhz[
                        latency
                    ] = curr_mhz
